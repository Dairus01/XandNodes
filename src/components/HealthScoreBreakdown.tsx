...
export function HealthScoreBreakdown({ health }: HealthScoreBreakdownProps) {
  ...
  // Weights for Xandeum network scoring
  const weights = {
    uptime: 0.25,
    availability: 0.25,
    performance: 0.17,
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
          Weighted calculation optimized for Xandeum network reliability
        </p>
      </CardHeader>
      <CardContent>
        <div className="space-y-6">
          {/* Total Score Display */}
          <div className={`text-center p-8 rounded-xl ${getScoreBg(totalScore)}/50 border-2 border-border`}>
          ...
          <div className="mt-6 p-4 bg-muted rounded-lg border border-border">
            ...
            <div className="font-mono text-xs sm:text-sm text-center text-gray-900 dark:text-gray-50 leading-relaxed">
              <div className="text-lg font-bold mb-1">
                {totalScore} =
              </div>
              <div className="space-y-1">
                <div>({uptimeHealth.toFixed(0)} × 25%) + ({availability.toFixed(0)} × 25%) + ({performanceHealth.toFixed(0)} × 17%)</div>
                <div>+ ({versionHealth.toFixed(0)} × 12.5%) + ({storageHealth.toFixed(0)} × 12.5%) + ({distribution.toFixed(0)} × 8%)</div>
              </div>
            </div>
          </div>

          {/* Reliability Focus Callout */}
          <div className="mt-4 p-3 bg-muted rounded-lg border border-border">
            <p className="text-xs text-foreground">
              <strong>Reliability-First Scoring:</strong> Optimized for high-reliability decentralized storage. Uptime and Availability are weighted highest (50% combined), prioritizing stable node operation.
            </p>
          </div>
        </div>
      </CardContent>
