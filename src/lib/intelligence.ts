...
export function calculateNetworkHealth(
  ...
  const avgUptime = nodes.length > 0
    ? nodes.reduce((sum, node) => sum + node.uptime, 0) / nodes.length
    : 0;
  const uptimeHealth = avgUptime;

  // 2. Availability (25%)
  const availability = (stats.activeNodes / stats.totalNodes) * 100;

  // 3. Performance (17%)
  const avgLatency = stats.avgLatency || 100;
  let performanceHealth: number;
  if (avgLatency < 100) {
    performanceHealth = 100;
  } else if (avgLatency < 300) {
    performanceHealth = 100 - ((avgLatency - 100) / 2);
  } else if (avgLatency < 500) {
    performanceHealth = 80 - ((avgLatency - 300) / 200) * 30;
  } else {
    performanceHealth = Math.max(50 - (avgLatency - 500) / 50, 0);
  }

  // 4. Version Health (12.5%)
  const versionHealth = calculateVersionHealth(nodes).percentage;

  // 5. Storage Health (12.5%)
  ...
  // Final Total Score (Weights sum to 100%)
  const totalScore =
    (uptimeHealth * 0.25) +
    (availability * 0.25) +
    (performanceHealth * 0.17) +
    (versionHealth * 0.125) +
    (storageHealth * 0.125) +
    (distribution * 0.08);
