...
export default function NodeDetailPage({ params }: NodeDetailPageProps) {
  ...
  // Use the resolved node's public key for metrics to ensure we target the exact same node
  // If node is not yet loaded, we might use id, but relying on publicKey is safer for consistency
  const metricsId = node?.publicKey;
  const { data: metrics, isLoading: metricsLoading } = usePNodeMetrics(metricsId || '', timeframe);
